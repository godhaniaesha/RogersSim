...
const Signup = () => {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      // const userData = await authService.signupWithGoogle(googleResponse.tokenId);
      // dispatch(signupSuccess(userData));
      // navigate('/login');
    } catch (error) {
      dispatch(signupFailure(error.message || 'Google signup failed'));
  ...
  const requestOtp = async () => {
    if (mobileNumber.match(/^[0-9]{10}$/)) {
      try {
        ...
        dispatch(signupFailure(error.message || 'Failed to send OTP'));
